...
- [ ] **O-9 — Identité Oyan** — nom (D18) et identité visuelle v1 « Argile » (D19) actés et consignés dans `docs/brand.md` §12-13 le 25/09/2026. Reste à faire :
  ...
  3. ✅ (partiel) Lisibilité à 16 px : le favicon ne contient que le signe (sans « yan »), donc pas de risque « Cyan » dans l'onglet. Reste à tester le **logo complet** quand il apparaîtra dans l'app, auprès de personnes qui ne connaissent pas le nom. 26/09/2026 : rendus 12-32 px examinés (risque surtout à 24-32 px, ouverture visible) ; **test de 5 secondes auprès de 8-12 personnes reporté par Florent** (page de test partageable proposée : logo 5 s puis « quel mot avez-vous lu ? », critère ≥ 8/10 à 24 px).
  4. ✅ Barre de terrain (26/09/2026, voir O-15 et D23) : le pipeline ne calculait pas de vraie répartition (forêt/eau = proximités qui se chevauchent : 81/340 boucles > 100 %). Nouvelle répartition `scenery.landcover` : UNE catégorie par point tous les 100 m, priorité ville > eau > forêt > espaces ouverts, 100 %. « Ville » : v1 « dans une zone bâtie OSM » **fausse** (îlots dessinés sans les rues : Gràcia 1 h à 5 % « ville », signalé par Florent ; barre masquée le temps de corriger) ; deux diagnostics `landcover_check` -> **au moins 5 bâtiments OSM à moins de ~75 m** (Gràcia 1 h : 89 % ; Collserola 23-33 %). Recalculée sur les boucles publiées par le mode `landcover` (sans GraphHopper, ~15 min, run #80), barre réactivée (760b729, run #81). Libellé « espaces ouverts » (et non « campagne » : inclut les parcs urbains comme le Besòs).
  5. ✅ Ton des textes (25/09/2026, 846d555, run #67) : pitch construit par l'app à partir des mêmes mesures (3 phrases au plus : sortie de ville, nature la plus présente, pistes / aucun feu / plus longue montée, ou alerte « routes principales : reste vigilant »), le reste dans « Tous les chiffres » (repliable, décimales à la française, toujours sans % de pente, D17) ; interface au tutoiement (`mentions.html` garde le vouvoiement, page juridique). Le pitch du pipeline (`o.pitch`) reste dans les données, non affiché.
  6. ✅ Fiche d'une boucle selon la charte (25/09/2026, 4805885) : libellé mono capitales, distance en Hanken Light 48 px, repères « D+ · durée · en ville », bouton GPX juste après le profil, alternatives puis « Tous les chiffres » ; vitesse moyenne retirée (chiffre de performance, `brand.md` §6). Puis (715e763, run #69) : la page défile en entier et la carte reste collée en haut (38 % de l'écran), la fiche gagne ~60 % de l'écran au lieu de ~20 % sur téléphone ; testé OK par Florent. **Remplacé le 26/09/2026 par la refonte O-17** (carte plein écran, panneau coulissant).
...
- [x] **O-14 — Proposer un départ voisin qui sort plus vite de la ville** (dev front / UX) — fait le 25/09/2026 (0a3c4c0, run #66, vérifié en ligne). Quand la boucle reste en ville ou n'en sort qu'après 5 km : « Plus vite dans la nature : depuis <voisin>, à ≈ N min à vélo, la boucle de 1 h 30 sort de la ville après X km. [Voir ce départ] ». Voisin : même durée et niveau, approche ≤ 25 min **montée comprise** (max(temps à plat, dénivelé / 400 m/h) ; altitude `alt_m` ajoutée à l'index par 4287a0f, publiée au run #65), sortie de ville ≥ 3 km plus tôt. Sans altitude connue, rien n'est proposé. Gràcia 1 h 30 : Horta-Guinardó à 11 min (36 % en ville au lieu de 67 %) ; le Tibidabo (430 m plus haut, ~64 min) n'est plus proposé. Suggestion présente dans 100 des 106 cas concernés (1-2 h) : durcir le seuil de 3 km si c'est trop fréquent à l'usage. **Testé OK sur le téléphone de Florent.**

- [x] **O-15 — Recalcul complet groupé** (dev pipeline) — fait le 25-26/09/2026, run `full` #76 : 877/877 départs recalculés en générateur v5 (mêmes départs, O-13), **263 min sur un budget de 300** (au prochain recalcul complet, envisager de découper). Contenu : répartition du terrain (O-9.4) ; tronçons répétés moitié moins pénalisés (D21) ; **pistes cyclables et chemins où le vélo est autorisé préférés** (demande de Florent : la piste du Besòs n'était jamais prise ; cause : GraphHopper « vélo de route » roule à 12 km/h sur `footway`+`bicycle=yes` et 4 km/h sur `footway` seul, et préfère les routes) : vitesse ×1,5 sur ces chemins, bonus ×1,3 hors ville / ×1,1 en ville (calme), ×1,1 (sportif), et ces chemins comptés dans « pistes cyclables ou voies vertes » (D23). Vérifié : depuis Gràcia en 1 h 30 la boucle remonte le Besòs (59 % de pistes et voies vertes).
  - **Reste ouvert** : (a) « Tu sors de la ville après X km » repose encore sur la densité GraphHopper (déborde de 1,5 km) alors que la barre compte le bâti : harmoniser la sortie de ville sur les bâtiments au prochain recalcul ; (b) « pistes et voies vertes » : quelques boucles vers Collserola sont devenues plus urbaines via les pistes cyclables de la ville (5/59 au diagnostic), à surveiller ; (c) contribution possible à OSM : retaguer en `cycleway` / `bicycle=designated` les tronçons du Besòs signalés comme tels sur le terrain.

- [x] **O-16 — Relief en niveau sportif (générateur v6)** (question de Florent, 26/09/2026 : plus aucune boucle par Collserola en 2 h sportif depuis Gràcia après O-15) — fait le 26/09/2026 (6e56f29, 7ff666f, voir D24). Cause : le bonus pistes cyclables du profil sportif attirait les boucles dans les pistes urbaines. Correction : profil sportif sans bonus pistes (correction de vitesse des chemins gardée) + **6 tirages « montée »** en sportif (modèle GraphHopper qui préfère les pentes ≥ 3 %) ; le score choisit toujours la boucle. Sonde `nature_check --probe` (5 départs urbains) : sportif jamais moins bon, 12/30 combinaisons améliorées. Résultat en ligne : Gràcia 2 h sportif « Équilibrée » 39,8 km, **888 m D+ par Collserola** (56 % en forêt), « Moins de relief » 132 m à côté.
  - **Coût du calcul** : ~83 s par départ en v6 (**+50 %**, et non +15 % comme estimé) ; un recalcul complet demande ~400 min, **au-delà d'un run (budget 320 min)** : désormais en **deux runs** (`full`, puis `full` + `reuse`).
  - **Incident** : run #84 coupé par GitHub après 3 h 05 (« The operation was canceled », pas une annulation de Florent ni le délai) ; run #85 : 700/877 départs recalculés, **les 177 autres ont disparu du site** (un run interrompu publiait seulement ce qu'il avait calculé, et le `full` suivant ne reprend que les départs publiés, O-13). Rétablis (6ba8824) : `scripts/restore_starts.json` (les 177 départs, extraits de l'archive du run #76) ajouté par `published_starts.py --extra`. **Désormais, un départ que le budget ne permet pas de recalculer garde sa version publiée, marquée `stale`** (jamais réutilisée comme à jour, recalculée par la relance avec `reuse`) : le site reste complet. Run `full` + `reuse` #89 (26/09/2026, 88 min) : **877/877 départs en ligne** en v6 (653 réutilisés, 224 calculés), aucun marqué `stale`, tous avec commune et altitude ; vérifié en ligne (`curl -I`).
  - `scripts/restore_starts.json` peut être supprimé une fois les 877 départs revenus en ligne (sans effet s'il reste).
- [x] **O-17 — Refonte de l'app** (maquette Claude Design « Oyan App Refonte », étape 5 ; rôle dev front / UX) — faite le 26/09/2026 en 4 commits (097c933, 467320e, 4763884, 1665c10 ; run #86) + retouches de Florent (a28e6ca, run #88) ; **testée OK sur le téléphone de Florent**. Toutes les informations gardées, réorganisées : carte plein écran ; réglages en carte flottante repliée en barre « Gràcia · Modéré · 2 h 00 » (loupe, icône de position au trait, allure en 3 segments, durée − / +, « Départ … Changer ») ; panneau coulissant à 3 hauteurs (glisser ou toucher la poignée) avec GPX toujours visible ; chiffres en colonnes ; légende de terrain 2 × 2 (« Bord d'eau ») ; points clés chiffrés avec alerte argile ; profil avec lecture « km · m » en en-tête ; suggestion de départ voisin en carte séparée ; dépliants à chevron ; carte de la variante avec ses écarts ; montées dans l'ordre de passage (« dont les 5 plus grosses » quand le pipeline n'en garde que 5). Carte : fond OpenFreeMap désaturé, autre boucle en pointillé galet étiquetée et interchangeable d'un toucher, flèches de sens, marqueurs point argile / point basalte à halo, bouton de position flottant. Écarts à la maquette : voir D25.
  - **Reporté** (données à ajouter au pipeline, via le mode `landcover`) : bande de terrain sous le profil (catégorie par point) ; « toucher un point clé surligne le tronçon » (possible pour la sortie de ville seulement).
- [ ] **O-18 — Options presque identiques d'une allure à l'autre** (remarque de Florent, 26/09/2026) : depuis Gràcia en 2 h, « Moins de relief » est la même boucle côtière en modéré (41,6 km, 126 m) et en sportif (41,9 km, 132 m) : sur le plat l'allure change peu la distance (~10 %) et il n'y a qu'une sortie plate. Proposition (au prochain recalcul complet) : quand l'option d'une allure recoupe à plus de ~80 % celle de l'allure voisine, la remplacer par une boucle d'une autre direction (le recouvrement est déjà mesuré par le générateur).
...
| 25/09/2026 | Dev front / UX (+ rédaction) | O-9.5 (ton Oyan : pitch en 3 phrases, « Tous les chiffres », tutoiement) et O-9.6 (fiche selon la charte, GPX remonté, sans vitesse moyenne) en ligne ; O-14 testé OK par Florent | O-9.3 (logo complet quand il apparaîtra) ; O-9.4 (barre de terrain : vérifier les % calculés) ; juriste (mentions, Komoot) ; au prochain recalcul complet : D21 |
| 25/09/2026 | Dev front / UX | Mise en page : page qui défile en entier, carte collée en haut (espace de lecture trop petit sur téléphone, signalé par Florent) ; testé OK | Idem ligne précédente |
| 26/09/2026 | Dev pipeline puis dev front / UX | O-15 (recalcul complet v5, run #76 : terrain, tronçons répétés, pistes et voies vertes préférées, le Besòs enfin emprunté) ; O-9.4 : barre de terrain, « ville » corrigée (bâtiments) après signalement de Florent, modes `landcover_check` / `landcover` (sans GraphHopper) ; « espaces ouverts » ; D23 ; O-9.3 reporté | Test du logo (O-9.3, Florent) ; harmoniser la sortie de ville sur le bâti au prochain recalcul ; juriste |
| 26/09/2026 | Dev pipeline puis dev front / UX | O-16 : relief en sportif (générateur v6 : profil sportif sans bonus pistes, tirages « montée »), Gràcia 2 h sportif par Collserola ; runs #84 (coupé par GitHub) et #85 (700/877, budget épuisé) : 177 départs perdus puis rétablis, départs non recalculés désormais gardés (`stale`) ; O-17 : refonte de l'app d'après la maquette (carte plein écran, panneau coulissant, réglages repliables), testée OK ; O-18 ouverte ; D24, D25 | O-18 et recalcul en deux runs au prochain recalcul complet ; harmoniser la sortie de ville sur le bâti ; bande de terrain sous le profil (pipeline) ; test du logo (O-9.3) ; juriste |
